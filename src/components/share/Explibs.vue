      ...
      <div class="weui-search-bar weui-search-bar_focusing">
        <form @submit.prevent="seachkey" class="weui-search-bar__form">
          <div class="weui-search-bar__box">
            ...
            <input type="search" class="weui-search-bar__input" v-model="form.searchContent" placeholder="请输入医院、科室或者医生姓名">
            <a href="javascript:;" v-show="form.searchContent" @click="form.searchContent=''" class="weui-icon-clear"></a>
          </div>
        </form>
        ...
        <dl class="filteresult">
          <dd v-for="d in list" class="rlist flex">
            ...
            <div class="f1 pinfo">
              ...
              <router-link class="selected" :to="{ name: params.routeName || 'main', params: Object.assign({ pinfo:d }, params) }" replace>选定</router-link>
            </div>
          </dd>
        </dl>
      </div>
      <div class="weui-tabbar">
        <a href="javascript:;" @click="pick1" class="weui-tabbar__item weui-flex acenter jcenter">
          <p class="weui-tabbar__label">按医院 <i class="iconfont icon-arr_up"></i></p>
          <label class="whitespace">{{hosname}}</label>
        </a>
        <a href="javascript:;" @click="pick2" class="weui-tabbar__item weui-flex acenter jcenter">
          <p class="weui-tabbar__label">按科室 <i class="iconfont icon-arr_up"></i></p>
          <label class="whitespace">{{depname}}</label>
        </a>
        <!--<a href="javascript:;" @click="pick3" class="weui-tabbar__item weui-flex acenter jcenter">
  ...
  export default {
    ...
    methods: {
      ...
      gaindata: function (sl) {
        var that = this;
        $.post('/propagate/doclistdata', this.form)
          .done(function (d) {
            var l = d.specials || [];
            if (l.length) {
