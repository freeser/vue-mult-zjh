    ...
    jsApiCall: function (config, ok, cancel, fail) {
        ...
        function jsApiCall() {
            WeixinJSBridge.invoke('getBrandWCPayRequest', config,
                function (res) {
                    res.err_msg.indexOf(':ok') > -1 && ('function' == typeof ok ? ok() : alert('支付成功'));
                    res.err_msg.indexOf(':cancel') > -1 && ('function' == typeof cancel ? cancel() : alert('用户已取消'));
